...
function StartState:drawMatch3Text(y)
    ...
    love.graphics.rectangle('fill', VIRTUAL_WIDTH / 2 - 180, VIRTUAL_HEIGHT / 2 + y - 11, 360, 58, 6)

    -- draw MATCH 3 text shadows
    love.graphics.setFont(gFonts['large'])
    self:drawTextShadow('MATCH 3 BY GROUP 8', VIRTUAL_HEIGHT / 2 + y)
    ...
    for i = 1, 14 do
        ...
        love.graphics.printf(self.letterTable[i][1], 0, VIRTUAL_HEIGHT / 2 + y,
            VIRTUAL_WIDTH / 2 + 45 + self.letterTable[i][2], 'center')
    end
end
